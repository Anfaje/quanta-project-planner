...
- [x] ~~**Decide: weekly vs daily hours grid.**~~ **Resolved — keeping weekly.** The spec docs (Reference Architecture §6.3 + §5 API description, and 13 test-case cells) were updated via tracked changes to describe the weekly grid. See [`SPEC_AUDIT.md`](SPEC_AUDIT.md) § "Resolved by decision".
...
- [x] **Generate proper Prisma migration files before any production data exists.** _Done (issue #15): `20260805000000_init` generated from the schema on a CI runner and committed; the release command is now `scripts/release.sh` → `prisma migrate deploy`, with a one-time P3005 self-baseline for the pre-migrations alpha database. Schema changes ship as migrations from here on._
